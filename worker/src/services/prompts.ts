...
export const STYLE_PROMPTS: Record<string, string> = {
  ...
  lego: `A high-resolution, sharp 3D CGI render of a complex Lego diorama. The entire scene, including characters, vehicles, and the environmental architecture, must be built entirely of recognizable, glossy plastic Lego bricks. Minifigures are present in rigid but dynamic poses. All surfaces must show the characteristic glossy sheen of injection-molded plastic, with prominent, visible "LEGO" studs on the tops. The lighting is stylized and dramatic, resembling a shot from a professional "brick film" or the official Lego movies, with shallow depth of field.`,
  cardboard: `A simple front-facing AI-generated cardboard diorama of the universe in the same handmade papercraft social-media explainer style as a DIY school project, set on a clean white seamless background with bright even studio lighting and soft shadows, using corrugated cardboard, kraft paper, chipboard, foam board, masking tape, visible glue marks, rough cut edges, marker lettering, and slightly imperfect handmade construction; make the scene centered, readable, playful, and satirical rather than sprawling or realistic, with the universe shown as a compact educational cardboard display featuring a large central layered cardboard cosmos board, die-cut planets with visible corrugation, cardboard stars, swirly cut-paper galaxies, simple nebula shapes, arrows, labels, speech-bubble style callouts, science-fair style signs, and symbolic cardboard props arranged like a visual explainer set, with bold black readable text integrated into the scene, keeping the composition clean and organized with strong silhouette shapes and obvious tactile materials; the style must look cute, crafty, and intentionally cheap in a smart way, not cinematic, not photorealistic, not dark, not moody, not dense or endless, not abstract fine art, not luxurious artisan craft, not stop-motion clay, not Pixar, not glossy plastic, and not a giant immersive environment, but clearly a staged cardboard papercraft diorama built for a short-form video explainer.`,
};
