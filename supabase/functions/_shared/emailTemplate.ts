// Shared email layout for all transactional MotionMax emails.
//
// Design ties to the website: dark panel-2 background (#0d1416 → #0a0e10
// gradient), cyan/gold radial accents, ink-coloured copy, gold CTA.
// Built table-only because Outlook desktop ignores CSS gradients on
// <div> backgrounds — the radial accents are emulated with a solid
// fallback colour and a ::before noop. Logo loads from motionmax.io.
//
// Usage:
//   buildEmail({
//     preheader: "Short preview text",
//     greeting: "Hi Daisy",
//     headline: "Welcome to MotionMax",
//     bodyHtml: "<p>Your account is ready…</p>",
//     cta: { label: "Open dashboard", href: "https://motionmax.io/app" },
//   })

const LOGO_URL = "https://motionmax.io/momaxlogo.png";
const SITE_URL = "https://motionmax.io";

export interface EmailLayoutInput {
  /** Hidden preview text shown by mail clients in the inbox preview row. */
  preheader?: string;
  /** Personalised greeting line, e.g. "Hi Daisy". Optional. */
  greeting?: string;
  /** Bold serif headline for the message. */
  headline: string;
  /** Pre-rendered, sanitised HTML for the body. Wrap copy in <p> tags. */
  bodyHtml: string;
  /** Optional primary CTA button. */
  cta?: { label: string; href: string };
  /** Footer note line shown below the divider. */
  footerNote?: string;
}

function escapeAttr(s: string): string {
  return s.replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

export function buildEmail(input: EmailLayoutInput): string {
  const {
    preheader = "",
    greeting,
    headline,
    bodyHtml,
    cta,
    footerNote = "Replying to this email reaches the MotionMax team.",
  } = input;

  const ctaBlock = cta
    ? `
      <tr>
        <td align="center" style="padding:8px 0 4px 0;">
          <a href="${escapeAttr(cta.href)}"
             style="display:inline-block;padding:13px 26px;border-radius:10px;background:linear-gradient(135deg,#F5B049 0%,#E4C875 100%);color:#0a0e10;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif;font-size:14px;letter-spacing:.2px;text-decoration:none;border:1px solid rgba(245,176,73,.45);">
            ${escapeAttr(cta.label)}
          </a>
        </td>
      </tr>`
    : "";

  const greetingBlock = greeting
    ? `<p style="margin:0 0 18px 0;font-size:15px;color:#ECEAE4;">${escapeAttr(greeting)}</p>`
    : "";

  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <meta name="color-scheme" content="dark light" />
  <meta name="supported-color-schemes" content="dark light" />
  <title>${escapeAttr(headline)}</title>
</head>
<body style="margin:0;padding:0;background:#0a0e10;color:#ECEAE4;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif;">
  <span style="display:none!important;visibility:hidden;mso-hide:all;font-size:1px;color:#0a0e10;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">
    ${escapeAttr(preheader)}
  </span>
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
         style="background:#0a0e10;background-image:radial-gradient(60% 70% at 30% 20%, rgba(20,200,204,.18), transparent 65%), radial-gradient(50% 60% at 80% 85%, rgba(245,176,73,.14), transparent 60%), linear-gradient(160deg,#0d1416,#0a0e10);">
    <tr>
      <td align="center" style="padding:32px 16px;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600"
               style="max-width:600px;width:100%;background:rgba(21,27,32,.85);border:1px solid rgba(255,255,255,.06);border-radius:16px;overflow:hidden;">
          <!-- Header with logo -->
          <tr>
            <td align="center" style="padding:28px 32px 8px 32px;border-bottom:1px solid rgba(255,255,255,.05);">
              <a href="${SITE_URL}" style="text-decoration:none;">
                <img src="${LOGO_URL}" width="140" alt="MotionMax"
                     style="display:block;border:0;outline:none;height:auto;max-width:140px;" />
              </a>
            </td>
          </tr>
          <!-- Body -->
          <tr>
            <td style="padding:28px 36px 8px 36px;">
              <h1 style="margin:0 0 16px 0;font-family:Georgia,'Playfair Display',serif;font-size:24px;line-height:1.25;color:#ECEAE4;font-weight:600;">
                ${escapeAttr(headline)}
              </h1>
              ${greetingBlock}
              <div style="font-size:14.5px;line-height:1.6;color:#C8CCCE;">
                ${bodyHtml}
              </div>
            </td>
          </tr>
          <!-- CTA -->
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
            ${ctaBlock}
          </table>
          <!-- Footer -->
          <tr>
            <td style="padding:24px 36px 28px 36px;border-top:1px solid rgba(255,255,255,.05);">
              <p style="margin:0 0 6px 0;font-size:12px;color:#8A9198;line-height:1.55;">
                ${escapeAttr(footerNote)}
              </p>
              <p style="margin:0;font-size:11px;color:#5A6268;">
                © MotionMax · <a href="${SITE_URL}" style="color:#14C8CC;text-decoration:none;">motionmax.io</a>
              </p>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>
</html>`;
}
