  ...
  const { data: projects = [], isLoading, isError } = useQuery<Project[]>({
    ...
    queryFn: async () => {
      ...
      if (missing.length > 0) {
        ...
        // (one scene object) instead of the entire jsonb array. Also
        // bounded to `missing.length * 2` rows so a project with many
        // regens doesn't unbound the response. Statement-timed out under
        // prod load when this pulled full scenes.
