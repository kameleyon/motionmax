 ...
 *
 * Why sync.so direct instead of Replicate-hosted sync/lipsync-2:
 ...
 *   - Median latency: 2-5 min for a 3-min input. P95 ~10 min. (vs.
 *     Replicate-hosted same model: median 8 min, P95 ~25 min.)
 *   - Webhook callbacks are first-class — opt in by passing `webhookUrl`
 *     to the submit body (not wired yet; see `submit-only` path comment).
 *
 * Models:
...
export async function generateLipsync(opts: LipsyncOptions): Promise<LipsyncResult> {
  ...
  // Best-effort cancel the job so we stop billing once we've given up.
...
/** POST /v2/cancel — best-effort abort to stop sync.so from billing on
 *  a result we'll never read. */
...
/** USD cost for a completed lipsync. sync.so bills by output duration. */
