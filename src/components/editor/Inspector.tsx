...
import { RotateCw, Loader2, Video, Image as ImageIcon, Wand2, Lock, UserPlus, AudioLines, Square, Undo2, History, X, Sparkles, Copy } from 'lucide-react';
    ...
    <div className="flex flex-col h-full relative">
      ...
      {!disabled && tab === 'captions' && sceneReady && (
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
          ...
          {/* SOCIAL COPY — generated by the script-gen LLM alongside
              the scenes (see worker/src/services/promptSections.ts
              SEO_SECTION). Persisted on generations.seo. The user
              copy-pastes these into YouTube, Instagram Reels, and
              TikTok. ONE set per video, platform-agnostic. */}
          {(() => {
            const seo = (state.generation as { seo?: { title?: string; subtitle?: string; description?: string; hashtags?: string[] } | null } | null)?.seo ?? null;
            const hashtagsLine = Array.isArray(seo?.hashtags)
              ? (seo!.hashtags as string[]).filter(t => typeof t === 'string' && t.trim().length > 0).join(' ')
              : '';
            ...
            const Row = ({ label, value, multiline }: { label: string; value: string; multiline: boolean }) => (
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="font-mono text-[9.5px] tracking-[0.14em] uppercase text-[#5A6268] font-medium">
                    {label}
                  </span>
                  <button
                    type="button"
                    ...
                    className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10.5px] text-[#8A9198] hover:text-[#14C8CC] hover:bg-white/[0.04] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                ...
                <div
                  className={cn(
                    'bg-[#0A0D0F] border border-white/[0.06] rounded-md px-2.5 py-2 text-[12px] text-[#ECEAE4] leading-[1.45]',
                    multiline ? 'whitespace-pre-wrap break-words max-h-44 overflow-y-auto' : 'truncate',
                  )}
                  title={!multiline ? value : undefined}
                >
                  {value || <span className="text-[#5A6268] italic">—</span>}
                </div>
              </div>
            );
            return (
              <section className="border-t border-white/[0.06] pt-5 space-y-3.5">
                <div>
                  <h5 className="font-mono text-[10px] tracking-[0.14em] uppercase text-[#5A6268] font-medium">
                    Social copy
                  </h5>
                  <p className="text-[11px] text-[#8A9198] leading-[1.45] mt-0.5">
                    For YouTube, Instagram Reels, and TikTok. Tap copy to paste-ready text.
                  </p>
                </div>
                {seo ? (
                  <>
                    <Row label="Title" value={typeof seo.title === 'string' ? seo.title : ''} multiline={false} />
                    <Row label="Subtitle" value={typeof seo.subtitle === 'string' ? seo.subtitle : ''} multiline={false} />
                    <Row label="Description" value={typeof seo.description === 'string' ? seo.description : ''} multiline={true} />
                    <Row label="Hashtags" value={hashtagsLine} multiline={false} />
                  </>
                ) : (
                  <p className="text-[11.5px] text-[#5A6268] leading-[1.5] italic">
                    SEO copy hasn't been generated for this video yet. New videos generated from now on will include it automatically.
                  </p>
                )}
              </section>
            );
