/**
 * generate-video Edge Function — refactored 2026-05-10 per audit C-4-2 (Arch C-A3).
 * Pipeline orchestration only. Per-stage logic lives in ./lib/*:
 *   - types.ts        (shared TS types: GenerationRequest, Scene, ScriptResponse, CostTracking)
 *   - validation.ts   (input validation, sanitization, content moderation, user-flagging)
 *   - llm.ts          (OpenRouter call helper + JSON extractor + model fallback)
 *   - logging.ts      (api_call_logs / system_logs sinks)
 *   - audioCodec.ts   (PCM/WAV codec, chunk-and-stitch engine, voiceover sanitisers,
 *                      Haitian Creole detector, single-chunk Chatterbox call)
 *
 * Still inline in this file (future extraction candidates, audit C-4-2 round 2):
 *   - TTS provider functions (Fish, Lemonfox, Gemini, ElevenLabs, OpenRouter, STS, unified router)
 *   - Image provider functions (Hypereal generate/edit, Replicate generate)
 *   - Phase handlers (script, audio, images, finalize, regenerate-audio, regenerate-image)
 *   - Tier check (isProOrEnterpriseTier) and style/format helpers
 *
 * NOTE: phase:"script" is currently routed to the Render worker (see SCRIPT_PHASE_DEPRECATED
 * guard around the main switch). The inline handleScriptPhase / handleSmartFlowScriptPhase
 * are retained for now but no longer reachable from this edge function.
 */
...
// Extracted modules
import type { GenerationRequest, Scene, ScriptResponse, CostTracking } from "./lib/types.ts";
import {
  validateGenerationRequest,
  validateNonNegativeInt,
  moderateContent,
  flagUserForViolation,
...
import { logApiCall, logSystemEvent } from "./lib/logging.ts";
import {
  pcmToWav,
  splitTextIntoChunks,
  extractPcmFromWav,
  stitchWavBuffers,
  callReplicateTTSChunk,
  sanitizeVoiceover,
  sanitizeForGeminiTTS,
  isHaitianCreole,
} from "./lib/audioCodec.ts";
...
// =====================================================================
// === Below: TTS providers, image providers, phase handlers (inline) ==
// === See lib/README.md for extraction roadmap.                       ==
// =====================================================================
...
// ============= IMAGE GENERATION WITH HYPEREAL (gemini-3-1-flash-t2i via https://api.hypereal.cloud/v1/images/generate) =============
