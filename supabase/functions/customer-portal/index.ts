...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Check if this is a manual / comp subscription (no Stripe customer)
    ...
    if (dbSubscription?.is_manual_subscription) {
      logStep("Manual / comp user - no Stripe portal available");
      ...
      return new Response(JSON.stringify({
        ...
        message: "Your subscription is managed directly by our team. Please contact support for billing inquiries."
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 422,
      });
