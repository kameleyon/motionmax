    ...
    <>
      <div className="flex items-baseline justify-between mt-9 mb-3.5">
        ...
        <span className="font-mono text-[11px] tracking-widest uppercase text-[#8A9198] cursor-pointer hover:text-[#14C8CC]">
          {recentProject ? `Auto-saved ${formatDistanceToNow(new Date(recentProject.updated_at), { addSuffix: true })}` : 'No recent projects'}
        </span>
      </div>
      ...
      <a className="border border-white/5 rounded-2xl bg-[#10151A] overflow-hidden grid grid-cols-[240px_1fr] gap-0 text-inherit hover:border-white/10 transition-colors" href={`/editor/${recentProject.id}`}>
        <div className="relative aspect-[4/3] bg-black overflow-hidden group">
          {recentProject.thumbnail_url ? (
            <img src={recentProject.thumbnail_url} alt={recentProject.title} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity" />
          ) : (
            <div className="absolute inset-0 bg-[#0a0a0b]" style={{ background: generateGradient(recentProject.id) }}></div>
          )}
          <div className="absolute left-[20%] top-[30%] w-[40%] h-[55%] rounded-full opacity-90" style={{ background: "radial-gradient(circle at 35% 30%,#d6b592,#6b462a 60%,transparent 85%)", display: recentProject.thumbnail_url ? 'none' : 'block' }}></div>
        ...
        <div className="p-[24px_28px] flex flex-col gap-2.5">
          ...
          <div className="flex gap-5 mt-auto pt-4 border-t border-white/5">
            ...
            <div className="flex flex-col gap-0.5"><span className="font-mono text-[10px] tracking-widest uppercase text-[#5A6268]">Language</span><span className="font-serif text-[17px] text-[#ECEAE4]">{recentProject.language || 'English'}</span></div>
      ...
      <div className="flex items-baseline justify-between mt-9 mb-3.5">
        ...
        <div className="flex gap-2">
          {['All', 'Cinematic', 'Explainer', 'Smart Flow'].map(type => (
            <button 
              key={type}
              onClick={() => setFilter(type)}
              className={`px-3 py-1 rounded-full text-[11px] font-medium transition-colors ${filter === type ? 'bg-[#14C8CC] text-black' : 'bg-[#10151A] text-[#8A9198] hover:text-white border border-white/10'}`}
            >
              {type}
            </button>
          ))}
      ...
      {/* Referral Banner */}
      <div className="mt-8 mb-4 border border-[#14C8CC]/20 bg-[#14C8CC]/10 rounded-xl p-5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-[#14C8CC]/20 flex items-center justify-center text-[#14C8CC]">
          ...
          <div>
            <h3 className="font-serif text-[18px] text-[#ECEAE4] m-0">Refer a friend, get 10,000 credits</h3>
            <p className="text-[13px] text-[#8A9198] m-0 mt-0.5">Share your link to get free generation credits when they sign up.</p>
          </div>
        </div>
        <button 
          onClick={() => {
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#10151A] hover:bg-[#1A2128] border border-white/10 rounded-lg text-[13px] font-medium text-[#ECEAE4] transition-colors"
        >
